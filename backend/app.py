"""
Flask application — Project GroundTruth Backend
=================================================
Single endpoint: POST /analyze

Accepts: multipart/form-data with field "image" (PNG/JPEG/WEBP)
Returns: JSON with full forensic analysis results + base64 visualisation images

Deployed to Railway. CORS enabled for all origins.
"""
...
# ── Flask setup ────────────────────────────────────────────────────────────────
app = Flask(__name__)
CORS(app)   # Allow requests from Vercel-hosted frontend

# Defensive memory gating to prevent Railway OOM container restarts
MAX_DIM   = 2048   
...
def health():
    return jsonify({"status": "online", "service": "GroundTruth Forensics API"})
